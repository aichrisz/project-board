...
export interface ActiveFocusSession {
  ...
  projectId: string;
  stepId?: string;
  startedAt: string;       // ISO UTC instant
  endsAt: string;          // ISO UTC instant; authoritative timer boundary
...
3. An active session references an existing project; its optional step references an unfinished step on that project.
...
5. All timestamp fields parse as finite ISO dates; `endedAt >= startedAt`; `endsAt > startedAt`.
...
9. IDs are generated by existing `createId`; imported duplicate record IDs are deduplicated deterministically.
...
At expiry or after **Stop session**, the panel enters the resolver state. It offers:

- **Mark step done** — available only for a still-unfinished linked step. It calls the existing context step toggle once, then finalizes a `completed` record.
